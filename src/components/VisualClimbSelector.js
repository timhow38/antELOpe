...
function VisualClimbSelector(props) {
    ...
    function areaToGridSection(area) {
        ...
        return <div key={safeName} className={className} style={{ gridArea: safeName }} onClick={() => setArea(safeName)}>{area.name}</div>
    }
...
function ClimbSelector(props) {
    ...
    console.log(climbs);
    return climbs && <div>{Object.entries(climbs).map(([_, climb]) => <ClimbSelectorCard climb={climb} successCallback={props.successCallback} />)}</div>
}
